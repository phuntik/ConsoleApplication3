...
void main() 
{
	int main_array[100], temp_array[10], sub_array[10], m = 0; //для первого задания
	int var_for_ex2 = 0; //для второго задания
	srand(time(NULL));
	for (int input_counter = 0;input_counter <= 99;input_counter++) //заполнение первичного массива
	{
	...
	sort(sub_array, 9);
	cout << "Ex.1: ";
	for (int output_counter = 0;output_counter <= 9;output_counter++) //вывод результата финальной сортировки
	{
